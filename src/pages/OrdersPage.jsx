  ...
  return (
    <div className="mx-auto max-w-6xl px-5 py-10 sm:px-8">
      <div className="flex flex-col justify-between gap-5 sm:flex-row sm:items-end">
        ...
        <Link to="/parcels/new" className="inline-flex w-fit bg-ink px-4 py-2.5 text-sm font-semibold text-paper hover:bg-slate-800">Make a parcel</Link>
      </div>
      {parcels.length === 0 && <div className="mt-10 border border-dashed border-slate-300 bg-white px-6 py-16 text-center"><p className="font-display text-lg font-semibold text-ink">No orders yet</p><p className="mt-2 text-sm text-fog">Create your first parcel to start tracking a delivery.</p></div>}
      <ul className="mt-8 grid gap-3 lg:grid-cols-2">
        {parcels.map((parcel) => (
          <li key={parcel.id}>
            <Link to={`/orders/${parcel.id}`} className="block border border-slate-200 bg-white p-5 transition hover:-translate-y-0.5 hover:border-ink hover:shadow-sm">
              ...
              <div className="mt-5 flex items-center justify-between border-t border-slate-100 pt-3 text-xs text-fog"><span>{parcel.currentLocation}</span><span>KSh {parcel.price.toLocaleString()}</span></div>
            </Link>
          </li>
        ))}
      </ul>
