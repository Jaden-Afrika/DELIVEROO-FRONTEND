...
export async function fetchParcel(id) { const { data } = await apiClient.get(`/parcels/${id}`); return data }
export async function cancelParcel(id) { const { data } = await apiClient.patch(`/parcels/${id}/cancel`); return data }
export async function updateParcelDestination(id, destination) { const { data } = await apiClient.patch(`/parcels/${id}/destination`, { destination }); return data }
